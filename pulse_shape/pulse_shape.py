...
class PulseShapeConfig(pydantic.BaseModel):
    ...
    normalization_type: Literal["None", "norm_2", "norm_1"]
...
class PulseShape:
    ...
    def generate_rect_pulse(self, sample_rate: float, symbol_time: float) -> np.ndarray:
        rect_pulse = np.ones(int(sample_rate * symbol_time))
        normalized_pulse = self.normalize_pulse(rect_pulse)
        return normalized_pulse

    def normalize_pulse(self, pulse: np.ndarray) -> np.ndarray:
        if self.config.normalization_type == "None":
            ...
            return normalized_pulse
        elif self.config.normalization_type == "norm_1":
            normalized_pulse = pulse / np.linalg.norm(pulse, ord=1)
            return normalized_pulse
        else:
